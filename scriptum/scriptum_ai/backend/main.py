...
from scriptum_ai.backend.api.v1 import chat, files, metrics, reviews, settings, websocket
from scriptum_ai.backend.core.database import close_db, init_db
from scriptum_ai.backend.core.exceptions import ScriptumError
from scriptum_ai.backend.core.logging import generate_request_id, request_id_ctx, setup_logging
...
def _find_frontend_dir() -> Path | None:
    ...
    Search order:
      1. Inside the scriptum_ai package (pip install with bundled frontend)
      2. Project-level frontend/out (development mode)
    """
    _pkg_root = Path(__file__).resolve().parent.parent  # scriptum_ai/
    candidates = [
        _pkg_root / "frontend" / "out",  # pip install (bundled in wheel)
        _pkg_root.parent / "frontend" / "out",  # development (scriptum/frontend/out)
    ]
