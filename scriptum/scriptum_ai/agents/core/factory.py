...
from scriptum_ai.agents.core.base import AgentConfig, AgentInterface
...
def create_agent(config: AgentConfig | None = None) -> AgentInterface:
    ...
    if framework == "crewai":
        from scriptum_ai.agents.core.adapters.crewai import CrewAIAdapter

        return CrewAIAdapter(config=config)

    if framework == "smolagents":
        from scriptum_ai.agents.core.adapters.smolagents import SmolAgentsAdapter
